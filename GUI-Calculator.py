...
def handle_button_click(clicked_button_text):
    ...
    if clicked_button_text == "=":
        try:
            # This aims to replace custom symbols with Pythons operators
            expression = current_text.replace("÷", "/").replace("×", "*")
            result = eval(expression)

            # Check if the result is a whole number by using is_integer
        ...
        # Convert number to a decimal by dividing it by 100
        try:
            number = float(current_text)
            result_variable.set(number / 100)
        except ValueError:
            result_variable.set("Error")
        ...
        try:
            number = float(current_text)
            result_variable.set(-number)
        except ValueError:
            result_variable.set("Error")
    
    else:
        result_variable.set(current_text + clicked_button_text)
...
result_entry = ttk.Entry(root, textvariable = result_variable, font = ("Helvetica", 24), justify= "right")
